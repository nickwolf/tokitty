...
{
  "accounts": [
    {"name": "personal", "config_dir": "\\\\wsl.localhost\\Ubuntu\\home\\<user>\\.claude", "coat": "orange_tabby"},
    {"name": "work",     "config_dir": "\\\\wsl.localhost\\Ubuntu\\home\\<user>\\.claude-work", "coat": "gray_tabby"}
  ]
}
```

Each entry's `config_dir` points at that account's Claude Code config directory (a WSL UNC path, a native path, whatever `--install-hooks` would target for that account). `coat` is an optional *legacy* seed: it names one of the five old presets (`orange_tabby`, `gray_tabby`, `black`, `white`, `calico`) and is translated to the equivalent colorway + pattern the first time that pane appears with no saved look (see [Customization](#customization) below). Omit it and a fresh pane rolls a random look instead. If `accounts.json` is missing, empty, or fails to parse, tokitty falls straight back to v1's automatic single-account credential resolution — nothing in that path changes.
...
Every choice you make is saved to `customization.json`, in the same per-user state directory as `position.json` (see [Two accounts](#two-accounts) above for the exact path per OS), and reloaded on the next launch. In single-account mode the file has one entry keyed `"default"`; in dual-account mode it's keyed by account name, so each pane keeps its own look and colors independently. It stores `colorway` + `pattern`; older files that stored a single `coat` name are migrated automatically on load. `accounts.json`'s `coat` field (see above) is only ever a *seed* — it sets the look the first time a pane appears with no stored customization yet. Once anything is saved to `customization.json`, that stored value wins over `accounts.json` from then on, even if you later edit `accounts.json`'s `coat`.
