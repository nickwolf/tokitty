...
- **Windows 11, native Python 3.13 (`pythonw.exe`), with Claude Code running inside WSL2** — the primary, recommended setup. The full pipeline (credential resolution → WSL fallback → live API polling → mood/wake-sequence logic → rendering) was run repeatedly end-to-end against a real account and exited clean every time. The window itself was **not** visually confirmed by the person/process that wrote this code — the dev environment used to build this couldn't render a screenshot of it (see note below) — so drag/topmost/pixel-crispness need a first real look from a human before this line gets upgraded from "ran correctly" to "looks right."
...
- The native Windows GUI window has not yet been visually confirmed by a human — it's been crash-tested extensively (real data, clean start/stop, no exceptions across dozens of runs) but nobody has actually looked at it on screen yet. Do that before trusting the "it works" claim for anything beyond "it doesn't crash."
