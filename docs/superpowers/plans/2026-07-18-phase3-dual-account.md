...
- [ ] **Step 4: Manual verification (human gate — spec requires it before merge)**
  - On Windows: create `accounts.json` with Nick's real personal (`\\wsl.localhost\Ubuntu\home\<user>\.claude`) + work (`...\.claude-work`) dirs; run `python -m tokitty`; confirm two panes, correct per-account numbers, drag/menu/position persistence, and (outside work hours) the work pane resting look
