...
3. **Windows-only WSL fallback**: if `sys.platform == "win32"` and step 2 found nothing, enumerate distros via `wsl.exe -l -q` and check each one's `/home/<user>/.claude/.credentials.json` (via `wsl.exe -d <distro> -- cat ...` or the `\\wsl.localhost\<distro>\...` UNC path). If **more than one** distro has a credentials file, that's ambiguous — don't guess; drop to the Confused/error state with a prompt to set the override instead of silently picking one. Every `wsl.exe` invocation needs `creationflags=CREATE_NO_WINDOW` (guarded with `getattr(subprocess, "CREATE_NO_WINDOW", 0)` for cross-platform test safety) — without it, each call flashes a visible console window on Windows since `wsl.exe` is a console app and `pythonw.exe` has none of its own to inherit. On a machine where the fallback fires every poll (as it does for the owner's own setup), this is very noticeable — found via the owner's first real visual test of the running app.
