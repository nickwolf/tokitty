...
Every rollout written by Codex 0.155 carries both `token_count` (an `event_msg` payload) and `token_usage_record`. `token_count.info.last_token_usage` and `token_usage_record.usage` were byte-identical in all six files spot-checked, at 81,829 / 27,044 / 121,986 / 129,534 / 53,593 / 167,662 total tokens respectively. **Summing both double-counts the entire bill.**
...
**Codex CLI 0.147.0 writes no `token_usage_record` at all.** Found by review on 2026-09-23. Rollouts from `cli_version` 0.147.0, which is what `codex exec` from WSL and the Claude Code Codex plugin run, carry only `token_count`: 179 such files modified in the last 30 days. Reading only records dropped their spend and still reported the scan as `ok`, and the 603.3M cross-check above shared the blind spot because it summed records too. Seven of them were modified in the last 8 days: 33.3M tokens in total, but only 1,034,342 fall inside the 7-day window (one file from 2026-09-22). The rest are from 2026-09-15 and show up in the month view. In those files `total_token_usage` is a running total that duplicate events repeat unchanged, and summing `last_token_usage` over the events where the total advances equals the file's final total on every file checked. The events carry no `turn_id`, but `turn_context` still names the model, so the scanner attributes each one to the nearest `turn_context` above it. That fallback applies only to a file with no records, so a 0.155 file is never counted twice. After the fix the 7-day readout is 618.8M tokens, and an independent recount using the same rules gives 618.9M, matching per model to 0.1M.
